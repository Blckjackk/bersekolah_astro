...
import { TestimoniService } from '../../lib/testimoni-service-fixed'
    ...
    const fetchTestimonials = async () => {
      try {
        ...
        const apiUrl = import.meta.env.PROD 
          ...
          : (import.meta.env.PUBLIC_API_BASE_URL || 'https://api.bersekolah.com/api') + '/testimoni';
        ...
        const filtered = arr.filter(item => item.status === 'active').map(item => ({
          ...
          image: TestimoniService.getImageUrl(item.foto_testimoni_url || item.foto_testimoni, 'testimoni')
        }));
                  ...
                  >
                    <div className="relative z-10 overflow-hidden bg-white rounded-2xl">
                      <img 
                        ...
                        onError={e => { e.target.src = TestimoniService.getImageUrl(); }}
                      />
                    </div>
