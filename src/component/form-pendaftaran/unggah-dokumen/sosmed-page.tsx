...
export default function DokumenSosmedPage() {
  ...
  const handlePreview = (uploadedDoc: UploadedDocument) => {
    ...
    const baseUrl = import.meta.env.PUBLIC_API_BASE_URL_NO_API || (import.meta.env.PROD ? 'https://api.bersekolah.com' : 'http://localhost:8000');
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Badge className="text-white bg-red-600 border-red-600 shadow-sm transition-colors cursor-pointer hover:bg-red-700">
                    <X className="mr-1 w-3 h-3" /> Ditolak
                  </Badge>
                </TooltipTrigger>
      ...
      case 'pending':
        return (
          <Badge className="text-white bg-yellow-600 border-yellow-600 shadow-sm hover:bg-yellow-700">
            <AlertCircle className="mr-1 w-3 h-3" /> Menunggu
          </Badge>
        )
      default:
        return (
          <Badge variant="outline" className="text-gray-600 bg-gray-50 border-gray-300 shadow-sm hover:bg-gray-100">
            <FileText className="mr-1 w-3 h-3" /> Belum Diunggah
          </Badge>
        )
    }
  }
  ...
  return (
    <div className="container px-4 py-4 mx-auto sm:px-6 lg:px-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="mb-2 text-xl font-bold sm:text-2xl">Dokumen Sosial Media</h1>
      ...
      <Alert className="mb-6 bg-blue-50 border-blue-200">
        ...
        <AlertDescription className="text-blue-700">
          ...
          <div className="flex flex-col gap-2 mt-3 sm:flex-row">
            <Button size="sm" variant="outline" className="w-full text-blue-600 border-blue-200 sm:w-auto" asChild>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0">
                    <CardTitle className="flex gap-2 items-center text-base sm:text-lg">
                      {docType.code === 'instagram_follow' && <Instagram className="flex-shrink-0 w-4 h-4 text-pink-500 sm:w-5 sm:h-5" />}
                      {docType.code === 'twibbon_post' && <FileText className="flex-shrink-0 w-4 h-4 text-blue-500 sm:w-5 sm:h-5" />}
                      <span className="truncate">{docType.name}</span>
                      {docType.is_required && <span className="flex-shrink-0 text-sm text-red-500">*</span>}
                    </CardTitle>
                  ...
                  {isUploaded && (
                    <CheckCircle2 className="flex-shrink-0 mt-1 w-5 h-5 text-green-600 sm:w-6 sm:h-6" />
                  )}
                </div>
              </CardHeader>
      ...
      <Dialog open={previewDialog} onOpenChange={setPreviewDialog}>
        <DialogContent className="max-w-6xl w-[95vw] h-[90vh] flex flex-col p-4 sm:p-6">
          ...
          {/* Preview Container */}
          <div className="overflow-hidden flex-1 min-h-0 bg-gray-50 rounded-lg border">
            {previewDoc && (
              <div className="relative w-full h-full">                {/* Check if it's a PDF */}
                {(previewDoc.file_name?.toLowerCase().endsWith('.pdf') || previewDoc.file_type === 'pdf') ? (
                  <div className="relative w-full h-full">
                    ...
                    <div 
                      id="pdf-fallback" 
                      className="hidden absolute inset-0 flex-col justify-center items-center p-8 text-center bg-gray-50"
                    >
                    ...
                    <div className="flex justify-center items-center p-4 min-h-full">
                      <img 
                        ...
                        onError={(e) => {
                          ...
                          errorDiv.className = 'flex flex-col justify-center items-center h-full text-gray-500'
                          errorDiv.innerHTML = `
                            <svg class="mb-2 w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                            <p class="mb-4 text-sm">Gambar tidak dapat ditampilkan</p>
                  ...
                  <div className="flex flex-col justify-center items-center p-8 min-h-full text-center">
                ...
                <div 
                  className="flex absolute inset-0 justify-center items-center backdrop-blur-sm bg-white/90" 
                  id={`loading-${previewDoc.id}`}
                  style={{ display: 'flex' }} // Show initially
                >
                  <div className="flex gap-3 items-center px-4 py-3 bg-white rounded-lg shadow-lg">
