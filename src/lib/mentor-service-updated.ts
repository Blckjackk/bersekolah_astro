import { z } from 'zod';
import { getStorageUrl, getDefaultImageUrl } from './utils/media-url';

// Environment-aware API URL
const getApiUrl = () => {
  return import.meta.env.PUBLIC_API_BASE_URL || 'https://api.bersekolah.com/api';
};
...
export const MentorService = {
  // Helper function untuk mendapatkan URL gambar mentor
  getImageUrl: (imagePath?: string): string => {
    if (!imagePath || imagePath === 'null' || imagePath === '') {
      return getDefaultImageUrl('mentor');
    }
    ...
    // If it's an assets path, use media URL
